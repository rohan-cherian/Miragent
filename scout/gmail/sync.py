...
def run_sync(
    *,
    ...
    """
    Pull new Gmail messages and insert tickets.
    ...
    if state and state.history_id:
        try:
            ...
            if exc.response is not None and exc.response.status_code == 404:
                ...
                messages = _bootstrap_list(client, store, mailbox, bootstrap_max, result)
            else:
                raise
    else:
        messages = _bootstrap_list(client, store, mailbox, bootstrap_max, result)
    ...
    for msg in messages:
        # Skip Google system noise optionally? Keep all for now; user can filter later.
...
def _bootstrap_list(
    ...
    result.mode = "list"
    # Prefer INBOX so system mail is included but we don't pull entire All Mail
    return client.fetch_messages(max_results=bootstrap_max, q="in:inbox")
