...
class KBArticle(Provenance, Base):
    """
    Knowledge Base solution article — one of ~15 per problem class,
    generated by scripts/generate_kb_articles.py and indexed into the
    same Qdrant collection as email chunks (scout/context/kb_index.py)
    so retrieve()/compile() can ground triage and drafting in it.

    Provenance.source_system is always "kb_article"; that string is also
    carried in the Qdrant payload and is what compile.py branches on to
    emit source_type="article" and read source_ts from this table instead
    of itr360.message. See schema/009_kb_article.sql.
    """
    ...
    __table_args__ = (
        ...
        UniqueConstraint("tenant_id", "category", "problem_class", "title"),
        Index("ix_itr360_kb_article_class", "category", "problem_class"),
        {"schema": "itr360"},
    )
    ...
    source: Mapped[str] = mapped_column(
        Text, nullable=False, default="llm_generated", server_default="llm_generated"
    )
